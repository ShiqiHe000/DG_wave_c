...
double Elem_load_random();

void Build_mapping_table();
void Build_mapping_table_weak();
...
/// @brief 
/// Calculate the sum of the local computational load.
/// The load is a random number in [1, 2]. For weak scaling test.
void Build_mapping_table_weak(){
	...
	lprefix_load[0] = Elem_load_random();
	temp = temp -> next;
	for(int k = 1; k < local::local_elem_num; ++k){
		lprefix_load[k] = Elem_load_random() + lprefix_load[k - 1];
...
/// @brief 
/// Generate radom element weights in [1, 10] for weak scaling. 
double Elem_load_random(){
	
	static const double load_min = 1.0;
	static const double load_max = 10.0;

	int load = rand() % 10 + 1;

	return ((double)load - load_min) / (load_max - load_min) + 1.0;
