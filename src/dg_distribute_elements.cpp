...
void Distribute_elem(){
	...
	if(mpi::rank == 0){
		...
		local::original_elem_num = SortMesh::num_of_elem;

		// if element number < processor number
		if(SortMesh::num_of_elem < mpi::num_proc){
			
			for(int ii = 0; ii < SortMesh::num_of_elem; ++ii){
			...
			average = SortMesh::num_of_elem / mpi::num_proc;
			last = SortMesh::num_of_elem - (mpi::num_proc -1) * average;

			local::elem_range[1] = average - 1;	// element numbering start with 0
			
			std::fill_n(local_elem_numbering, mpi::num_proc - 1, average);
			
			local_ele_numbering[mpi::num_proc - 1] = last;
